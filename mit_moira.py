...
class Moira(object):
    ...
    def __init__(self, cert_path, key_path, proxy_id="", url=WSDL_URL):
        ...
        # 01/06/2022 - TURN OFF TLS VALIDATION (for testing purposes only)
        transport.session.verify = True
    ...
    def list_user_info(self, username):

        """
        Look up a given user's attributes.
        
        Args:
            username(str): The Kerberos ID of the user
            
        Returns:
            dict: attributes of the user
        """

        result = self.client.service.getUserAttributes(username, self.proxy_id)
